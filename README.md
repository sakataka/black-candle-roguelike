...
bun run simulate:batch -- --preset standard --profile
...
出力は `tmp/sim-reports/*.json` と `tmp/sim-reports/*.md` に保存され、毎回 `tmp/sim-reports/latest.json` と `tmp/sim-reports/latest.md` も更新されます。`tmp/` はGit管理外です。Markdown summary では、先頭の `PDCA Alerts`、`Comparison`、`Top Run Regressions` を見て、悪化候補の label、role、seed を先に絞ります。その後、職業ごとの到達階、勝敗、死亡原因、低HPターン、探索停滞、既知罠踏み、拾得、戦闘、階層移動、アイテム使用、AI改善ヒントを確認します。各runと集計には `elapsedMs`、平均ms/run、runs/sec も含めているため、パラメータ探索が重くなった時に TypeScript のまま最適化するか、Rust/C などの別実装を検討する材料にできます。
